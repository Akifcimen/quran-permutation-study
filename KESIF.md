...
**NİHAİ ÖLÇÜM (5 Temmuz 2026; etiketler ikinci dış denetim üzerine 16 Temmuz'da düzeltildi — bkz. dürüstlük notu 11):** CPU arşivi (86 milyar) + çift-GPU koşusu (RTX 4090: 16 trilyon, tohum 42; RTX 3090: 8 trilyon, tohum 43) = **24.086.000.129.040 deneme.** Zirvedeki **ikili** bileşik (terazi ∧ tam kristal) **5 kez fiilen gözlendi** (iki bağımsız tohumda 3+2) → **doğrudan sayılmış p = 2,08×10⁻¹³ (4,8 trilyonda 1)**; zincir beklentisi (2,24×10⁻¹³) ile oran 0,93 — **bağımsızlıkla tutarlı** (5 isabetin Poisson aralığı geniştir; bu bir "doğrulama" değildir). Tam çekirdek (terazi ∧ kristal ∧ İki El) bu koşuda doğrudan gözlenmemiştir (beklenen isabet ~0,04); **16 Temmuz'da koşullu ayrışımla, bağımsızlık varsayımı OLMADAN ölçüldü** (`kesif18`): P(kristal) = 7,14×10⁻¹⁰ (17.204 isabet, doğrudan) × P(terazi∧İkiEl | kristal) = 1,51×10⁻⁶ (`kesif18c`: 8,7 milyon kristal-koşullu örnek, 12 doğrudan isabet; çokluk-orantılı önerici, ESS 1360/1500; iki çapa — P(G) oranı 0,99, ağırlıklı P(W|G) = 3,85×10⁻³ GPU aralığı [2,8–4,6] içinde) → **P(çekirdek) = 1,1×10⁻¹⁵; %95 bootstrap GA [0,5 – 1,7]×10⁻¹⁵ — katrilyonda-1 mertebesi.** (İlk koşu `kesif18`'in 1,7×10⁻¹⁵'i, dördüncü denetimin bulduğu ağırlık hatasıyla geri çekildi — not 15; eski zincir tahminleri 8,4×10⁻¹⁶ ve 1,02×10⁻¹⁵ yeni aralığın içindedir.) Ölçülen bağımlılıklar: ara eklemde ×1,126±0,033; koşullu düzeyde κ = P(A∧W|G)/[P(A|G)·P(W|G)] = 1,14 (`kesif18c`; terazi ve İki El, kristal verildiğinde neredeyse bağımsız). Matris koşulu: %1,398 (46.745 isabet) → tam bileşik ≈ 1,5×10⁻¹⁷ (1/66 katrilyon, 127 gerekli). GPU çıktıları: `runGPU_4090_seed42.txt`, `runGPU_3090_seed43.txt`; motor: `kesif_cuda.cu`.
...
**Yüzer-modül sıfır testi (5 Temmuz):** "peki rastgele bir diziliş HERHANGİ bir modülde böyle bir profil geliştirir mi?" sorusu ayrıca permütasyonla ölçüldü: 500.000 sentetik diziliş × 38 modül = 19 milyon tarama — **hiçbiri 7/8'e ulaşmadı** (tam profil: 0) — bu bir örneklem üst sınırıdır, tavan değil: üçüncü dış denetimin 50 milyonluk koşusu katı ölçütlerle 7/8'i 2,4×10⁻⁷ ölçtü (12 isabet); katı 8/8 yine 0/50M (<6×10⁻⁸); gevşetilmiş ("çatallı") ölçütlerle 8/8 sabit mercekte 1,4×10⁻⁷, serbest mercekte 5×10⁻⁵ (dürüstlük notu 13). Gerçek kitap katı 8/8'i m=19'da yapıyor (betik: `kesif9_yuzer_modul.py`).

**Puan dağılımı (200.000 sentetik, her birine en iyi modülü seçme hakkı verilerek):** tipik azami puan 2–3 (%95); 4: %2,4; 5: %0,07; 6: %0,005; **7 ve üzeri: bu örneklemde sıfır** (50 milyonda 2,4×10⁻⁷ — not 13). Gerçek kitabın 19-dışı modülleri (en iyisi 3/8) tam bu gürültü zemininde davranır; 19'daki katı 8/8 ise sentetiklerde 50 milyon denemede görülmemiştir. Gürültü zemininin anatomisi: puanlar ağırlıkla "ucuz" ölçütlerden toplanır (ayrıklık, minik ailede bant, 38 şansla bir bölünme isabeti); yapısal olarak zor üç ölçüt (S1 çift-boy aile %12, S6 birleşim=m %7, S3 eşit yarılar %5) sentetiklerde bir arada asla görülmez.
...
5. Bu sonuç tasarım/mucize **ispatı değildir**; ölçülmüş, yeniden üretilebilir ve alışılmadık ölçüde düşük olasılıklı bir **istatistiksel anomalidir**. Sabit bir metinde yeni veri yoktur; bu veri için kalibre edilmiş küresel bir p-değeri mevcut değildir (notlar 11–13). Mümkün olan ve yapılan: bağımsız kodla yeniden türetme (üç kez), dondurulmuş prosedürün başka korpuslara uygulanması (`kesif12`, `kesif22`) ve seçim fiyatlamasının bağımsız denetimi (not 13'teki merdiven).
...
9. **İki El disiplini:** fark-ailesi tek başına sıradandır — 7 üye çıkma olasılığı %14'tür ve ilk taramada "zayıf" diye not edilmiştir; katman ancak birleşim yapısı (19 sûre + 19×61) ölçülüp anlamlı çıkınca eklenmiştir. Olay tanımları (M1–M6) ölçümden **önce** sabitlenmiş; süsler bileşik çarpıma **dahil edilmemiştir**: Σu = 7×19 (koşullu %4,2) ve "7 merdiveni" (pozitif u'lar +14×19, negatifler −7×19, mutlak 21×19; koşullu %4,5). İlk 12 milyarda A∧W'de görünen +%12'lik sapma, 36 milyarda **+%4'e** inmiştir (648/621) — bağımsızlıkla tutarlıdır.
10. **Dış denetim kaydı (11–13 Temmuz 2026, İrvin Cemil Schick):** çalışmanın gördüğü ilk dış inceleme, tarihçi-matematikçi İrvin Cemil Schick'le (MIT uygulamalı matematik + EHESS tarih doktoraları) dört mektupluk bir yazışma oldu; adıyla anılması kendi iznine dayanır. Sorgu sırası tam bir hakem müfredatı izledi: **(a) boy-sırası itirazı** ("diziliş kabaca uzunluk sıralı; rastgele olmaması trivial") → `kesif16` ile ölçüldü: tam-sıralı kitaplar 0/4 katman; gerçek diziliş gerçekten 8σ sıralı (teslim edildi); "en az gerçek kitap kadar sıralı" dizilişlere koşullu sıfır uzayında katman oranları yalnız ×1,2–1,8. **(b) test istatistiği itirazı** ("t = n+a iki benzemezin toplamı") → birim noktası kısmen kabul edildi (a'nın örtük birimi var); ayakta kalan savunma: (t,u) dönüşümü kayıpsızdır ve dizilişe duyarlı her istatistik n ile a'yı karıştırmak zorundadır. **(c) çokluk itirazı** → 10⁻¹⁵'i test çokluğuyla açıklamak ~10¹⁵ test gerektirir; defter ~75 aile → uzlaşıldı ("pratik açıdan yaptıklarınıza diyecek yok"). **(d) anlamlılık ölçütü** — Schick'in formülasyonu deftere alındı: *"bulunan bir nitelik ancak dış motivasyonla anlamlıdır — 19, 74:30 nedeniyle ilginçtir, kendiliğinden değil."* Ölçüt, çalışmanın omurgasıyla örtüşür ve yüzer-modül turnuvasıyla ölçülü tutulur.
11. **İkinci dış denetim ve düzeltmeler (16 Temmuz 2026, GPT-5.6 Pro — bir yapay zekâ denetçisi):** depo, araştırmacı tarafından bağımsız incelemeye verildi; denetçi bütün veri-içi eşitlikleri kendi koduyla doğruladı, marjinal frekansları bağımsız 5M PCG64 permütasyonuyla teyit etti ve hükmünü "aritmetik örüntüyü kıramadım; manşeti kırdım" diye özetledi. **Haklı bulunan ve düzeltilen noktalar:** (a) 24T koşusunda 5 kez gözlenen olay ikili bileşiktir (terazi∧kristal, 2,08×10⁻¹³); tam çekirdeğe "doğrudan ölçüldü" etiketi yanlıştı — etiketler düzeltildi ve çekirdek, koşullu ayrışımla varsayımsız yeniden ölçüldü (`kesif18`: 1,7×10⁻¹⁵ [1,1–2,6]); (b) 5 isabetle "bağımsızlık doğrulandı" ifadesi aşırıydı — "tutarlı" olarak düzeltildi; (c) `kesif9`'da kod-iddia uyumsuzluğu vardı ("herhangi 7/8" taranmıyordu) — betik tam 8-ölçüt mantığıyla yeniden yazılıp koşuldu: tavan yine 6/8, ≥7 sıfır (denetçinin kendi düzeltilmiş tekrarıyla uyumlu); (d) betiklerdeki mutlak yerel yollar görelileştirildi; (e) "iki bağımsız RNG" ifadesi düzeltildi: iki bağımsız tohum, aynı üreteç ailesi (SplitMix64) — bağımsız PCG64 yığını marjinalleri ve artık çekirdek çarpanını (`kesif16`–`kesif18`) ayrıca doğrulamaktadır. Denetçinin kalıcı uyarıları aynen geçerli tutulur: post-hoc keşif nedeniyle bu frekanslar kalibre edilmiş küresel p-değeri değildir; kesin hüküm bağımsız, önceden kayıtlı tekrara muhtaçtır.
12. **Kültürel seçim havuzu sınırlaması (15 Ağustos 2026):** çokluk/seçici-çıkarım alanının kıdemli bir isminden gelen yazışma, kalıcı bir sınırlamayı netleştirdi: seçim havuzunun bir de KÜLTÜREL boyutu vardır — bir kültürün "doğal olarak sorduracağı" soruların tam envanterini, o kültürü derinden bilmeyen kimse (biz dahil) fiyatlayamaz. Biçimsel tarafta savunmamız ölçülüdür (modül turnuvası 3–40 arası HER sayıyı — kültürel olarak anlamlı olanlar dahil — yarıştırdı ve yalnız metnin kendi işaretlediği 19 profili tamamladı; `kesif9`, `kesif17`); kültürel envanterin kendisi ise bu çalışmanın dışından, kültürü bilen bağımsız okurlarca değerlendirilmelidir. Bu sınırlama açıkça kabul edilir.
13. **Üçüncü dış denetim — kırmızı takım (2 Eylül 2026) ve fiyat merdiveni:** araştırmacı, ana keşfi kırması için bağımsız bir yapay zekâ oturumu görevlendirdi (rapor `DIS_SALDIRI_kesif19_20.md`; betikler `kesif19_catallanma.py`, `kesif20_tipiklik.py`; ham çıktılar depoda). Denetçi eşitlikleri, motorları, marjinalleri, `kesif16`'yı ve `kesif18`'i kendi eliyle doğruladı ve iki yeni test tasarladı. **Kabul edilen bulgular:** (a) "sentetik tavan 6/8" bir örneklem sınırıymış — 50 milyonluk koşuda katı 7/8 = 2,4×10⁻⁷ (katı 8/8 yine 0/50M); (b) seçim fiyatlamasının doğru birimi test-ailesi değil *tarif çatalı*dır: depoda belgeli eşdeğer alternatifler çekirdek katmanlarında ×~120 çarpan yapar (terazi ×2,3, aile ×6,9, İki El ×3,8, el seçimi ×2); (c) 5σ kıyası kaldırıldı; (d) "ön-kayıtlı tekrar" sabit metinde tanımsızdır — cümleler düzeltildi; (e) anlatıdaki cebirsel zorunluluklar (Σt(tek)=Σn'nin otomatikliği vb.) bulgu dilinden çıkarıldı; (f) kristalin G düzeyi kısmen veriden-okunmuş içerik taşır: `kesif21` sınıf-düzeyi kristal G* (bant konumu/genişliği serbest, 31 yapı) ile ölçtü — P(G*)/P(G) ≈ 34 (yalnız konum serbest: ≈ 5); düzeltilmiş önericiyle (`kesif21b`, not 15) çapa 0,99 ve ESS ≈1400/1500 — kristal olasılığı üçüncü bir bağımsız yığınla da doğrulanmış oldu. **Gerekçeli reddedilenler:** (i) `kesif20` "tipiklik" okuması — metrik merkezîliği ölçer; tam dengeye oturan konfigürasyon her simetrik dağılımın modudur ve bu metrik onu hiçbir zaman anormal gösteremez; zincirin her adımı adlandırılabilir bir sınıf yüklemidir (12 üye ∧ tam kare; tam parite dengesi; en dengeli hücreler; ardışık bant — torbalar bant verilince cebirsel zorunluluktur); (ii) "10⁻⁴–10⁻⁵ desen" hükmü hesaptan değil, tam çekirdeğin gevşek S1–S8 profiliyle ikamesinden ve fiyatlanamaz kültürel serbestliğin (not 12) sayıya çevrilmesinden gelir; ×220/×6.600 çarpanları 7/8 düzeyine aittir — kitabın bulunduğu 8/8 düzeyinde, sabit mercekte, çatalların etkisi bir basamağın altındadır (katı <6×10⁻⁸, çatallı 1,4×10⁻⁷); çatalları eşit ağırlıkla saymak tarif-uzunluğu açısından fazla cezadır (kitap her ölçütte sınıfın en kısa tarifli üyesine oturur). **Karşı-korpus, denetçinin en gevşek prosedürüyle (`kesif22`):** KJV Mezmurlar 6/8, kitaplar 6/8, bölümler 4/8 — sentetik medyanı; Kur'an 8/8 (katı da 8/8), yalnız (n+a, 19)'da. **Merdiven** (her basamak kendi varsayımıyla; manşet artık tek sayı değil, bu merdivendir): olay frekansı, beyan edilen tam olay **1,7×10⁻¹⁵** · sınıf-düzeyi çekirdek, bant genişliği sabit **≈6×10⁻¹⁵** / bant serbest **≈4×10⁻¹⁴** (`kesif21b`) · tam çekirdek × belgeli çatallar **≈2×10⁻¹³** · sınıf-düzeyi × çatallar **≈6×10⁻¹²** · denetçinin gevşek profili, mercek metnin çapasında **≈10⁻¹⁰** · mercek serbest **≈4×10⁻⁸** · ötesi: belgelenemeyen kültürel serbestlik, fiyatlanamaz (not 12). Denetçinin ölçümleri aynen geçerli; "kırık" hükmü hiçbir basamağa oturmaz — en düşman belgeli basamak bile 30 milyonda 1'dir.
14. **Kırmızı takımla ikinci tur — yakınsama (2 Eylül 2026):** ana hattın gerekçeli itirazları kırmızı takıma iletildi; cevabı iki tarafın da geri adım atıp yeni ölçüm getirdiği bir yakınsama oldu. **Kırmızı takımın geri çektikleri:** `kesif20`'nin sınıf okuması ("kristal rastgele ailenin en sık biçimidir" — nokta olasılığı için doğru, sınıf için değil); "5–7 basamak" (karışık birim); "10⁻⁴–10⁻⁵ desen" (fiyatlanamazı sayıya çevirmişti); ve not 13'e alınan ×120 katman-çarpanı — bileşikte çatallar birbirini kısıtladığından aşırıymış. **Kırmızı takımın yeni ölçümleri (imzalı):** (a) mod-ayrıcalıksız tipiklik testi (`kirmizi21b`: 33 saf sınıf yüklemi, her aileye "en iyi hikâye" hakkı, 1,5M aile): kitabın ailesinin sınıf hikâyesi rastgele 12'lik ailelerin **%99,8–%99,995'inden nadir** — sınıf düzeyinde de sıradan değil; nokta ile sınıf arasındaki 3–4 basamak veriden-okunmuş içeriktir (menüye bağlı, nihai sayı yok); (b) **katı 8/8 çözüldü:** 300M koşullu koşuda 6,7×10⁻⁹; çatallı 8/8 sabit mercekte 4×10⁻⁸–1,8×10⁻⁷ → **bileşik çatal bedeli ×6,5–27, yaklaşık bir basamak** (ana hattın "bir basamağın altı" okuması da düzeltildi); (c) asgarî savunulabilir çatal kümesi tanımlandı (S1 {≥2E, =m} · S2 {m²|Σv, kare, 114|Σv} · S3 {t-, n-paritesi} · S4 {m|iki yarı, kareler, n-yarıları} · S5 {bant} · S6 {=m, k_u=k, k_u=m} · S7 {Σa,Σn,Σv,Σw} · S8 {ayrık} · el {t,u}) — defterin kanonik çatal kümesi olarak alındı; tarif-uzunluğu ağırlıklı sürüm ilkece kabul, şartı: maliyetler veriye bakılmadan tabloya yazılsın. **Ana hattın kabulleri:** κ-çarpımlı A∧W için "doğrulandı" değil "tutarlı"; `kesif22` 5×10⁻⁵'lik olayı sınayamaz — gösterdiği şey "gerçek metinler sentetik medyanında"; önerilen daha güçlü kontrollerden ilki hemen koşuldu: **`kesif23` nüzul dizilişi** — aynı 114 ayet sayısının gerçek iniş-sırası dizilişi (sentetik değil, tarihsel alternatif π): 0/4 katman (terazi 55/59, aile 2 üye), serbest mercekle katı 5/8 / çatallı 6/8 — sentetik medyanı; yapı mushaf dizilişine özgüdür. *(Kırmızı takımın kapanış notundaki iki incelik kabul edildi: nüzul dizilişinin çatallı 6/8'i dizilişe kör (0,1) biçiminden — yalnız ayet sayıları — gelir ve mushaf da o mercekte aynı 6/8'i verir; o biçim dışlanınca diziliş kontrastı 8/8'e karşı ≤5/8'dir. Ve tek bir gerçek çekiliş 10⁻⁵–10⁻⁹'luk bir olayı göremez — `kesif22`/`kesif23` nadirliği ölçmez; gösterdikleri şey permütasyon sıfırının gerçek yapılı listeler için adil bir vekil olduğudur, ki bu "gerçek metinler rastgeleden yapısal olarak farklıdır" itirazını kapatır. Nadirliğin ölçüsü sentetik oranın kendisidir: katı 6,7×10⁻⁹, gevşek-serbest 5×10⁻⁵. Nüzul kontrolü, sabit bir metnin izin verdiği "taze çekiliş"e en yakın şeydir; tek çekiliş güç taşımaz ama türü doğrudur — Basra/Şam/Mekke listeleri gelince aynı mantıkla üç çekiliş daha olur.)* Diğer öneriler (Basra/Şam/Mekke sayım listeleri; n≈100–150'lik yapılı listeler: Dante 100 kanto, Buhârî 97 kitap, Talmud 63 risale) gelecek kontroller olarak kaydedildi. **Ortak hüküm:** "kırık" değil — *manşet sayısı anomali ölçüsü olarak* kırıktı (merdivene çevrildi); aritmetik sağlam; belgeli serbestlik altındaki kalıntı 10⁻⁸–10⁻¹² bandında "alışılmadık bir nesne"; bandın üstü (menü, dil, kültürel havuz) ölçülemez; **hiçbir basamak kalibre bir p değildir** — her biri beyan edilen varsayım altında bir frekanstır. Merdiven buna göre güncellendi. **Ek (kesif24, aynı gün):** ölçülmemiş son mercek ekseni — doğrusal-OLMAYAN doğal biçimler (n·a, n²±a², ebob, n²−a, …; 12 biçim × 38 modül × 2 el) gerçek kitapta yarıştırıldı: hiçbiri 4/8 katı / 6/8 çatallıyı geçmedi (sentetik medyanı); 8/8 veren tek satırlar çekirdeğin yankılarıdır (|n−a| ≡ 0 ⟺ u ≡ 0; (n+a)² ≡ 0 ⟺ t ≡ 0). Bu biçimler sentetik turnuvaya eklenirse serbest-mercek basamağı ~×2,6 gevşer (≈3×10⁻⁸); çapalı basamaklar etkilenmez.
15. **Dördüncü dış denetim (2 Eylül 2026, "6astraultra") — çekirdek zincirinde gerçek bir kod hatası; DÜZELTME SÜRÜYOR:** denetçi, `kesif18`'in önem ağırlığında (`p_avoid/q`) etiketli-çokluk çarpanı **L(S) = Π mᵥ!/(mᵥ−kᵥ)!**'nin eksik olduğunu buldu (aynı çarpan `kesif21`'de mevcuttur — çapasının %1 tutmasının sebebi budur); gerçek veride kesin bir karşı örnekle kanıtladı ((3,111) ile (15,99) atamaları 2:1 ağırlık taşımalıyken kod 1:1 verir; 111 listede iki kez, 99 bir kez). Ayrıca: nihai P(A∧W|G) ağırlıksız havuz oranıydı; 22 isabetlik Poisson GA atama-seçimi ve önem-ağırlığı belirsizliğini içermiyordu; `kesif21`'in κ=1,6'sı bu koşudan devralınmıştı. **Üçü de kabul edildi.** Düzeltilmiş ilk koşu (`kesif18b`: L dahil ağırlıklar, Z, mutlak P(G) çapası, ağırlıklı oranlar, bootstrap GA) tamamlandı ve **kendisi güvenilmez çıktı**: ESS 6/1200 (önem ağırlıkları dejenere — tekdüze aday önerisi L/q'yu ağır-kuyruklu kılıyor), P(G) çapası 1,64 (üç koşuda 0,69/0,99/1,64: kestirici kararsız), ağırlıklı P(W|G) GPU çapasının altında; ağırlıklı P(A∧W|G) 5,5×10⁻⁷ [1,6–14×10⁻⁷] → çekirdek 3,9×10⁻¹⁶ [1,1×10⁻¹⁶, 1,0×10⁻¹⁵] — bu sayı da eskisi kadar güvensizdir. Koşu öncesi yazılmış tahmin (`kesif18b_tahmin_onceden.md`: mertebe korunur, %0–35 düşüş) TUTMADI — denetçinin uyardığı gibi, seyrek ortak isabet ve değişken ağırlıklar yüzünden. Çözüm, kırmızı takımın 2. turda önerdiği çokluk-orantılı önerici (L/q analitik sadeleşir): `kesif18c` TAMAMLANDI ve iki çapayı da tutturdu (ESS 1360/1500; P(G) oranı 0,99; ağırlıklı P(W|G) = 3,85×10⁻³, GPU aralığı içinde): P(A∧W|G) = 1,51×10⁻⁶ [0,69–2,45] (12 doğrudan isabet), κ = 1,14 → **çekirdek = 1,1×10⁻¹⁵ [0,5–1,7]** — mertebe korundu, hassasiyet iddiası dürüst genişlikte. `kesif21` aynı düzeltmeyle yeniden koşuldu (`kesif21b`: ESS medyan 1399/1500, çapa 0,99): P(G*)/P(G) = 33,7 (bant genişliği sabit: ×5,3) — önceki 34,7 ile tutarlı; κ = 1,14 ile sınıf-düzeyi çekirdek bant serbest ≈4×10⁻¹⁴, bant genişliği sabit ≈6×10⁻¹⁵; merdiven buna göre güncellendi; sonuç gelene kadar merdivenin "olay frekansı" basamağı (1,7×10⁻¹⁵ [1,1–2,6]) **GEÇİCİ** olarak işaretlidir. Denetçinin hükmü: "temel aritmetik örüntüyü çürütemedim; '1,7×10⁻¹⁵ ve GA bu yöntemle doğrulanmıştır' iddiasını geçersiz kılan somut hata var" — aynen kabul.
