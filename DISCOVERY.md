...
- Independence: Layer 1 ∧ W = 648 observed / 621 expected (+4% — consistent with independence).
...
| **Codex-independent core: Layer 1 ∧ crystal ∧ Two Hands** | **≈ 1.1×10⁻¹⁵ — quadrillion order, measured WITHOUT the independence assumption** via conditional decomposition (`kesif18c`): P(crystal) directly counted (17,204 hits) × P(balance∧TwoHands \| crystal) = 1.51×10⁻⁶ (8.7M crystal-conditional samples, 12 direct hits; multiplicity-proportional proposal, ESS 1360/1500; two GPU anchors met: P(G) ratio 0.99, weighted P(W\|G) inside [2.8–4.6]×10⁻³); 95% bootstrap CI [0.5–1.7]×10⁻¹⁵; κ = 1.14. The first run's 1.7×10⁻¹⁵ was withdrawn after the fourth review found a weighting error (note 15); the earlier chained estimates (0.8–1.0×10⁻¹⁵) lie inside the new CI | **none** |
...
**Floating-modulus null test (July 5):** the question "would a random arrangement develop such a profile at ANY modulus?" was measured by permutation: 500,000 synthetic arrangements × 38 moduli = 19 million scans — **none reached 7/8** (full profiles: 0) — a sample-size bound, not a ceiling: the third external review's 50-million-trial run measured strict 7/8 at 2.4×10⁻⁷ (12 hits); strict 8/8 again 0/50M (<6×10⁻⁸); under loosened ("forked") criteria 8/8 appears at 1.4×10⁻⁷ fixed-lens / 5×10⁻⁵ free-lens (honesty note 13). The real book scores strict 8/8 at m=19 (script: `kesif9_yuzer_modul.py`).

**Score distribution (200,000 synthetics, each granted its best modulus):** typical maximum score 2–3 (95%); 4: 2.4%; 5: 0.07%; 6: 0.005%; **7 and above: zero in this sample** (2.4×10⁻⁷ at 50 million — note 13). The real book's non-19 moduli (best rival 3/8) behave exactly at this noise floor; its strict 8/8 at 19 was not seen in 50 million synthetic trials. Anatomy of the noise floor: scores are mostly assembled from "cheap" criteria (disjointness, a trivial band in a tiny family, one divisibility hit among 38 chances); the three structurally hard criteria (S1 double-size family 12%, S6 union=m 7%, S3 equal halves 5%) never co-occur in synthetics.
...
5. This result is **not a proof** of design or miracle; it is a measured, reproducible, and unusually low-probability **statistical anomaly**. A fixed text yields no fresh data; no calibrated global p-value exists for it (notes 11–13). What is possible, and has been done: independent re-derivation with independent code (three times), application of the frozen procedure to other corpora (`kesif12`, `kesif22`), and independent audit of the selection pricing (the ladder in note 13).
...
11. **Second external review and corrections (July 16, 2026, GPT-5.6 Pro — an AI reviewer):** the repository was submitted by the researcher for independent review; the reviewer verified every in-data equality with its own code, confirmed the marginal frequencies with an independent 5M-permutation PCG64 run, and summarized its verdict as "I could not break the arithmetic pattern; I broke the headline." **Points found valid and corrected:** (a) the event observed 5 times in the 24T run is the summit *pair* (balance∧crystal, 2.08×10⁻¹³); labeling the full core "directly measured" was wrong — labels were corrected, and the core was re-measured without the independence assumption via conditional decomposition (`kesif18`: 1.7×10⁻¹⁵ [1.1–2.6]); (b) "independence confirmed" from 5 hits was an overclaim — corrected to "consistent with"; (c) `kesif9` had a claim–code mismatch (it did not scan "any 7 of 8") — rewritten with full 8-criterion logic and rerun: the ceiling is again 6/8, ≥7 zero (matching the reviewer's own corrected rerun); (d) absolute local paths in scripts were made relative; (e) the "two independent RNGs" phrasing was corrected: two independent seeds, same generator family (SplitMix64) — an independent PCG64 stack now additionally verifies the marginals and the core's conditional factor (`kesif16`–`kesif18`). The reviewer's standing caveats remain in force verbatim: due to post-hoc discovery these frequencies are not a calibrated global p-value; a definitive verdict requires independent pre-registered replication.
12. **Cultural selection-pool limitation (August 15, 2026):** correspondence with a senior figure in multiplicity/selective inference sharpened a standing limitation: the selection pool also has a CULTURAL dimension — the full inventory of questions a culture would "naturally ask" cannot be priced by anyone (ourselves included) who does not know that culture deeply. On the formal side our defense is measured (the modulus tournament raced EVERY number from 3 to 40 — culturally salient ones included — and only 19, the number the text itself flags, completed the profile; `kesif9`, `kesif17`); the cultural inventory itself must be assessed from outside this study, by independent readers who know the culture. This limitation is acknowledged openly.
13. **Third external review — red team (September 2, 2026) and the price ladder:** the researcher commissioned an independent AI session to break the main finding (report `DIS_SALDIRI_kesif19_20.md`; scripts `kesif19_catallanma.py`, `kesif20_tipiklik.py`; raw outputs in the repository). The reviewer independently verified the equalities, the engines, the marginals, `kesif16` and `kesif18`, and designed two new tests. **Findings accepted:** (a) the "synthetic ceiling 6/8" was a sample-size bound — a 50-million-trial run measured strict 7/8 at 2.4×10⁻⁷ (strict 8/8 still 0/50M); (b) the correct unit of selection pricing is not the test family but the *description fork*: equivalent alternatives documented in the repository multiply the core layers by ×~120 (balance ×2.3, family ×6.9, Two Hands ×3.8, hand choice ×2); (c) the 5σ comparison was removed; (d) "pre-registered replication" is undefined for a fixed text — the sentences were corrected; (e) algebraic necessities in the narrative (the automaticity of Σt(odd)=Σn etc.) were removed from the language of findings; (f) the crystal's G level partly carries data-read content: `kesif21` measured this with a class-level crystal G* (band position/width free, 31 structures) — P(G*)/P(G) ≈ 34 (position only: ≈ 5); with the corrected proposal (`kesif21b`, note 15) the anchor is 0.99 at ESS ≈1400/1500 — the crystal probability is thereby reproduced by a third independent stack. **Rejected with reasons:** (i) the `kesif20` "typicality" reading — the metric measures centrality; the exactly balanced configuration is the mode of every symmetric distribution and this metric can never flag it as anomalous; every step of the chain is a nameable class predicate (12 members ∧ perfect square; exact parity balance; maximally balanced cells; consecutive band — the bags are algebraically forced once the band is given); (ii) the "10⁻⁴–10⁻⁵ pattern" verdict comes not from a computation but from replacing the exact core with the loose S1–S8 profile and from converting unpriceable cultural freedom (note 12) into a number; the ×220/×6,600 multipliers belong to the 7/8 level — at the 8/8 level the book actually occupies, fixed-lens, forks change the rate by less than an order of magnitude (strict <6×10⁻⁸, forked 1.4×10⁻⁷); counting forks with equal weight over-charges in description-length terms (the book sits on the shortest-description member of every class). **Counter-corpus under the reviewer's loosest procedure (`kesif22`):** KJV Psalms 6/8, books 6/8, chapters 4/8 — the synthetic median; the Quran 8/8 (strict 8/8 too), only at (n+a, 19). **The ladder** (each rung with its own assumption; the headline is no longer a single number but this ladder): event frequency of the stated exact event **1.7×10⁻¹⁵** · class-level core, band width fixed **≈6×10⁻¹⁵** / band free **≈4×10⁻¹⁴** (`kesif21b`) · exact core × documented forks **≈2×10⁻¹³** · class-level × forks **≈6×10⁻¹²** · the reviewer's loose profile, lens anchored by the text **≈10⁻¹⁰** · lens free **≈4×10⁻⁸** · beyond: undocumentable cultural freedom, unpriceable (note 12). The reviewer's measurements stand as made; the verdict "broken" fits no rung — even the most hostile documented rung is 1 in 30 million.
14. **Second round with the red team — convergence (September 2, 2026):** the main line's reasoned objections were relayed to the red team; its reply was a convergence in which both sides withdrew claims and brought new measurements. **Withdrawn by the red team:** the class reading of `kesif20` ("the crystal is a random family's most frequent form" — true for point probability, not for classes); "5–7 orders" (mixed units); "a 10⁻⁴–10⁻⁵ pattern" (it had converted the unpriceable into a number); and the ×120 layer multiplier adopted in note 13 — an over-estimate, since forks constrain each other in the composite. **New red-team measurements (signed):** (a) a mode-neutral typicality test (`kirmizi21b`: 33 pure class predicates, every family granted its own "best story", 1.5M families): the book's family has a class story rarer than **99.8–99.995% of random 12-member families** — not ordinary at class level either; the 3–4 orders between point and class are data-read content (menu-dependent, no final number); (b) **strict 8/8 resolved:** 6.7×10⁻⁹ in a 300M conditional run; forked 8/8 at fixed lens 4×10⁻⁸–1.8×10⁻⁷ → **composite fork cost ×6.5–27, about one order of magnitude** (the main line's "under one order" reading is corrected too); (c) a minimal defensible fork set defined (S1 {≥2E, =m} · S2 {m²|Σv, square, 114|Σv} · S3 {t-, n-parity} · S4 {m|both halves, squares, n-halves} · S5 {band} · S6 {=m, k_u=k, k_u=m} · S7 {Σa,Σn,Σv,Σw} · S8 {disjoint} · hand {t,u}) — adopted as the ledger's canonical fork set; the description-length-weighted version accepted in principle, on condition that costs are tabulated before looking at data. **Accepted by the main line:** for the κ-product A∧W say "consistent" rather than "verified"; `kesif22` cannot test a 5×10⁻⁵ event — what it shows is "real texts sit at the synthetic median"; the first of the stronger controls proposed was run immediately: **`kesif23`, revelation order** — the same 114 verse counts in their real revelation-order arrangement (not synthetic: a historical alternative π): 0/4 layers (balance 55/59, family of 2), strict 5/8 / forked 6/8 under free lens — the synthetic median; the structure is specific to the mushaf arrangement. *(Two refinements from the red team's closing note are adopted: the revelation order's forked 6/8 comes from the arrangement-blind form (0,1) — verse counts only — at which the mushaf scores the same 6/8; excluding that form, the arrangement contrast is 8/8 versus ≤5/8. And a single real draw cannot see a 10⁻⁵–10⁻⁹ event — `kesif22`/`kesif23` do not measure rarity; what they show is that the permutation null is a fair proxy for real structured lists, which closes the objection "real texts differ structurally from random ones". The measure of rarity is the synthetic rate itself: strict 6.7×10⁻⁹, loose-free 5×10⁻⁵. The revelation-order control is the closest a fixed text allows to a "fresh draw"; one draw carries no power, but it is the right kind of control — the Basran/Damascene/Meccan lists would add three more draws of the same kind.)* The other proposals (Basran/Damascene/Meccan counting lists; structured lists with n≈100–150: Dante's 100 cantos, Bukhari's 97 books, the Talmud's 63 tractates) are recorded as future controls. **Joint verdict:** not "broken" — *the headline number as a measure of anomaly* was broken (it became a ladder); the arithmetic is sound; the residual under documented freedom, in the 10⁻⁸–10⁻¹² band, is "an unusual object"; above that band (menu, language, cultural pool) nothing is measurable; **no rung is a calibrated p** — each is a frequency under a stated assumption. The ladder is updated accordingly. **Addendum (kesif24, same day):** the last unmeasured lens axis — NON-linear natural forms (n·a, n²±a², gcd, n²−a, …; 12 forms × 38 moduli × 2 hands) raced on the real book: none exceeds 4/8 strict / 6/8 forked (the synthetic median); the only 8/8 rows are echoes of the core (|n−a| ≡ 0 ⟺ u ≡ 0; (n+a)² ≡ 0 ⟺ t ≡ 0). Adding these forms to the synthetic tournament would loosen the free-lens rung by ~×2.6 (≈3×10⁻⁸); the anchored rungs are unaffected.
15. **Fourth external review (September 2, 2026, "6astraultra") — a real code error in the core chain; CORRECTION IN PROGRESS:** the reviewer found that `kesif18`'s importance weight (`p_avoid/q`) omits the labeled-multiplicity factor **L(S) = Π mᵥ!/(mᵥ−kᵥ)!** (the same factor is present in `kesif21` — which is why its anchor held to 1%); proven by an exact counter-example on the real data (the assignments (3,111) and (15,99) should carry 2:1 weight, the code gives 1:1; 111 occurs twice in the list, 99 once). Further: the final P(A∧W|G) was the unweighted pooled rate; the 22-hit Poisson CI ignored assignment-selection and importance-weight uncertainty; `kesif21`'s κ=1.6 was inherited from this run. **All three accepted.** The first corrected run (`kesif18b`: L-inclusive weights, Z, absolute P(G) anchor, weighted rates, bootstrap CI) completed and **proved unreliable itself**: ESS 6/1200 (degenerate importance weights — the uniform candidate proposal makes L/q heavy-tailed), P(G) anchor 1.64 (0.69/0.99/1.64 across three runs: the estimator is unstable), weighted P(W|G) below the GPU anchor; weighted P(A∧W|G) 5.5×10⁻⁷ [1.6–14×10⁻⁷] → core 3.9×10⁻¹⁶ [1.1×10⁻¹⁶, 1.0×10⁻¹⁵] — a number as untrustworthy as the old one. The prediction written before the run (`kesif18b_tahmin_onceden.md`: order preserved, 0–35% drop) FAILED — for the reason the reviewer warned of: sparse joint hits and variable weights. The remedy is the red team's round-2 suggestion, a multiplicity-proportional proposal (L/q simplifies analytically): `kesif18c` has COMPLETED and met both anchors (ESS 1360/1500; P(G) ratio 0.99; weighted P(W|G) = 3.85×10⁻³, inside the GPU interval): P(A∧W|G) = 1.51×10⁻⁶ [0.69–2.45] (12 direct hits), κ = 1.14 → **core = 1.1×10⁻¹⁵ [0.5–1.7]** — order preserved, precision claim now honestly wide. `kesif21` was re-run with the same fix (`kesif21b`: ESS median 1399/1500, anchor 0.99): P(G*)/P(G) = 33.7 (band width fixed: ×5.3) — consistent with the earlier 34.7; with κ = 1.14 the class-level core is ≈4×10⁻¹⁴ (band free) / ≈6×10⁻¹⁵ (width fixed); the ladder is updated accordingly; until it completes, the ladder's "event frequency" rung (1.7×10⁻¹⁵ [1.1–2.6]) is marked **PROVISIONAL**. The reviewer's verdict — "I could not refute the basic arithmetic pattern; there is a concrete error that invalidates the claim that 1.7×10⁻¹⁵ and its CI were verified by this method" — is accepted as stated.
